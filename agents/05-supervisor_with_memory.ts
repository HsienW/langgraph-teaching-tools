...
// ============================================================================
// 使用 INMEMORYSTORE 的 LONG-TERM MEMORY
// ============================================================================
//
// 此檔案以 04 為基礎，加入 persistent memory management。
// 系統現在可以跨 conversations 記住 user preferences！
//
// 核心概念：
// - Long-term memory：將資訊保存到單次 conversation 之外
// - InMemoryStore：LangGraph 用於 memory 的 key-value store
// - Namespace-based organization：依 user ID 儲存資料
// - Memory lifecycle：Loading → Using → Updating
//
// WORKFLOW：
// 1. Verify customer（與 04 相同）
// 2. Load memory：擷取此 customer 已儲存的 preferences
// 3. Supervisor：使用 preferences 讓 responses 個人化
// 4. Create memory：分析 conversation 並更新已儲存的 preferences
//
// 為什麼這很重要：
// - Personalization：agent 會記住 user preferences
// - Context preservation：過去 conversations 可提供目前對話的上下文
// - Better UX：users 不需要重複提供相同資訊
...
//
// STRUCTURED MEMORY
// 我們定義 schema，描述每位 user 需要記住哪些資訊。
// 這能確保不同 users 的 memory structure 保持一致。
...
// 此 prompt 會指示 LLM 分析 conversations，並擷取
// 值得記住的資訊，例如 music preferences。
...
// ============================================================================
//
// 輔助函數：格式化 memory data，供顯示或 prompts 使用
...
//
// 此 graph 有 5 個 nodes：
// 1. verify_info：Customer verification（與 04 相同）
// 2. human_input：在 interrupt 期間收集 user input（與 04 相同）
// 3. load_memory：新增，從 InMemoryStore 載入已儲存的 preferences
// 4. supervisor：使用已載入的 memory context 執行 supervisor
// 5. create_memory：新增，分析 conversation 並更新已儲存的 preferences
...
// VERIFY INFO NODE
// 與 04 相同，用來驗證 customer identity
...
// HUMAN INPUT NODE
// 與 04 相同，在 interrupt 期間要求 user input
...
async function loadMemory(state: AgentState) {
  ...
  // InMemoryStore 使用 namespaces 以階層方式組織資料
  // 這會讓每位 user 的資料彼此分離
  ...
  // 將 memory 加入 conversation context，讓 supervisor 可以使用
...
// CREATE MEMORY NODE
// 分析 conversation 並更新已儲存的 user preferences
async function createMemory(state: AgentState) {
  const userId = state.customerId?.toString();
  if (!userId) {
    return {};  // 沒有 user ID 就無法儲存 memory
  }

  const namespace = ["memory_profile", userId];
  const formattedMemory = state.loadedMemory || "";

  // 使用 LLM 分析 conversation 並擷取 preferences
  ...
  // 取得符合 UserProfileSchema 的 structured output
  // 注意：為了相容 Anthropic models，我們同時包含 SystemMessage 和 HumanMessage
  // 因為 Anthropic models 至少需要一則 user message
...
// 建立最終含有 memory 的 multi-agent graph
//
// COMPLETE GRAPH FLOW：
// START → verify_info → [conditional]
//                           ↓ 尚未驗證
//                       human_input →（loop 回 verify_info）
//                           ↓ 已驗證
//                       load_memory → supervisor → create_memory → END
//
// MEMORY LIFECYCLE：
// 1. verification 後，載入既有 memory
// 2. Memory 會加入 conversation context
// 3. Supervisor 使用 memory 讓 responses 個人化
// 4. supervisor 回覆後，分析 conversation
// 5. 使用新發現的 preferences 更新 memory
//
// 這會建立一個完整 loop，讓 agent 從每次 interaction 中學習！
...
export const graph = multiAgentFinal.compile({
  ...
  store: inMemoryStore,  // Long-term：user preferences 和 memory
});
